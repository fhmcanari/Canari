...
class _RestaurantPageState extends State<RestaurantPage> {
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<ShopCubit, ShopStates>(
        ...
        builder: (context, state) {
          ...
          if (cubit.store != null) {
            double firstBreackPoint = resturantInfoHeight +
                15 +
                (119 * cubit.store['menus'][0]['products'].length);
            breackPoints.add(firstBreackPoint);
            for (var i = 1; i < cubit.store['menus'].length; i++) {
              double breackPoint = breackPoints.last +
                  15 +
                  (119 * cubit.store['menus'][i]['products'].length);
              breackPoints.add(breackPoint);
            }
            scrollController.addListener(() {
              for (var i = 0; i < cubit.store['menus'].length; i++) {
                if (i == 0) {
                  if ((scrollController.offset < breackPoints.first) &
                      (selectCategoryIndex != 0)) {
                    setState(() {
                      selectCategoryIndex = 0;
                    });
                  }
                } else if ((breackPoints[i - 1] <= scrollController.offset) &
                    (scrollController.offset < breackPoints[i])) {
            ...
            child: Directionality(
              ...
              child: Scaffold(
                  ...
                  bottomNavigationBar: state
                          is! GetResturantPageDataLoadingState
                      ? cubit.getCartItem() != 0
                          ? dataService.itemsCart[0]['productStoreId'] ==
                                  widget.id
                              ? Container(
                                                              ...
                                                              ListView.builder(
                                                                  ...
                                                                  itemBuilder:
                                                                      (BuildContext
                                                                          ...
                                                                          productindex) {
                                                                    var product =
                                                                        cubit.store['menus'][index]['products']
                                                                            [
                                                                            productindex];
                                                                    var contain = dataService
                                                                        .itemsCart
                                                                        .where((element) =>
                                                                            element['id'] ==
                                                                            product['id'])
  ...
  Widget buildProduct(
      product, cubit, StoreName, StoreId, deliveryPrice, storeStatus) {
    return Column(
